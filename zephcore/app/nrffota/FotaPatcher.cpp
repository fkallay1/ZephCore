...
static bool fota_verify_old_fw() {
    ...
    if (all_zero || st->old_fw_size == 0) {
        FOTA_DEBUG_PRINTLN("[OLD] old_sha256 unknown — base check skipped");
        return true;
    }
    if (st->old_fw_size > (APP_FLASH_END - fota_running_fw_base())) {
        FOTA_DEBUG_PRINTLN("[OLD] ERROR: old_fw_size %lu > app window", (unsigned long)st->old_fw_size);
        return false;
    }
    ...
    FOTA_DEBUG_PRINT("[OLD] expected old_sha256 ="); print_sha16(st->old_sha256); FOTA_DEBUG_PRINTLN("...");
    if (memcmp(h, st->old_sha256, 32) != 0) {
        FOTA_DEBUG_PRINTLN("[OLD] BASE MISMATCH — running FW != patch old! NOT overwriting.");
        FOTA_DEBUG_PRINTLN("[OLD]   Generate the patch against the current firmware.");
        return false;
    }
    FOTA_DEBUG_PRINTLN("[OLD] base FW matches the patch");
    return true;
}
...
static bool ensure_flasher_written() {
    if (memcmp((const void*)FLASHER_CODE_ADDR, flasher_code, FLASHER_CODE_SIZE) == 0) {
        FOTA_DEBUG_PRINTLN("[FLASH] Flasher is up to date");
        return true;
    }
    FOTA_DEBUG_PRINTLN("[FLASH] Writing flasher to 0xEB000...");
    nvmc_erase_page(FLASHER_CODE_ADDR);
    nvmc_write_words(FLASHER_CODE_ADDR, (const uint32_t*)flasher_code, (FLASHER_CODE_SIZE + 3) / 4);
    if (memcmp((const void*)FLASHER_CODE_ADDR, flasher_code, FLASHER_CODE_SIZE) != 0) {
        FOTA_DEBUG_PRINTLN("[FLASH] ERROR: flasher verification failed!");
        return false;
    }
    FOTA_DEBUG_PRINTLN("[FLASH] Flasher written OK");
    return true;
}
...
bool fota_patch_to_file(char* err, size_t err_sz) {
    const FotaState* st = fota_get_state();
    FOTA_DEBUG_PRINTLN("[PATCH] Test: SHA256 verify (no flash)...");
    ...
    if (!patch_buf) { FOTA_DEBUG_PRINTLN("[PATCH] patch unavailable"); set_err(err, err_sz, FOTA_TXT_VFY_NO_PATCH_DATA); return false; }
    ...
    if (magic == ZPATCH_MAGIC) {
        ...
        if (!ps) {
            free(patch_buf);
            FOTA_DEBUG_PRINTLN("[PATCH] malloc puff_stream failed");
            set_err(err, err_sz, FOTA_TXT_VFY_OOM_PUFF);
            return false;
        }
        ...
        if (!hpatchi_inplace_open(ps, patch_zlib_read,
            ...
            FOTA_DEBUG_PRINTLN("[PATCH] Invalid HPatchLite header (ZLIB) ps_err=%d", ps->error);
            free(ps); free(patch_buf);
            set_err(err, err_sz, FOTA_TXT_VFY_BAD_ZLIB_HDR);
            return false;
        }
        ...
        if (extra_safe > FOTA_MAX_EXTRA_SAFE) {
            FOTA_DEBUG_PRINTLN("[PATCH] extra_safe %lu > max %lu — the flasher would REJECT the patch (0xE5)!", (unsigned long)extra_safe, (unsigned long)FOTA_MAX_EXTRA_SAFE);
            free(ps); free(patch_buf);
            char b[48]; snprintf(b, sizeof(b), FOTA_TXT_VFY_EXTRASAFE_FMT, (unsigned long)extra_safe, (unsigned long)FOTA_MAX_EXTRA_SAFE);
        ...
        if (!cache) { free(ps); free(patch_buf); FOTA_DEBUG_PRINTLN("[PATCH] scratch buffer unavailable"); set_err(err, err_sz, FOTA_TXT_VFY_SCRATCH_BUSY); return false; }
        ...
        if (!ok) {
            if (ps_err) {
                FOTA_DEBUG_PRINTLN("[PATCH] Decompression failed: err=%d", ps_err);
                char b[40]; snprintf(b, sizeof(b), FOTA_TXT_VFY_DECOMP_ERR_FMT, ps_err);
                set_err(err, err_sz, b);
            } else {
                FOTA_DEBUG_PRINTLN("[PATCH] HPatchLite FAILED (ZLIB)");
                set_err(err, err_sz, FOTA_TXT_VFY_HPATCH_FAILED_ZLIB);
            }
            return false;
        }
        ...
        if (all_zero) {
            FOTA_DEBUG_PRINTLN("[PATCH] Expected SHA256 unknown — verify manually");
            set_err(err, err_sz, FOTA_TXT_VFY_EXP_SHA_UNKNOWN);
            return true;
        }
        if (memcmp(result_sha, st->new_sha256, 32) != 0) {
            FOTA_DEBUG_PRINT("[PATCH] SHA256 MISMATCH  exp="); print_sha16(st->new_sha256); FOTA_DEBUG_PRINTLN("...");
            set_err(err, err_sz, FOTA_TXT_VFY_SHA_MISMATCH);
            return false;
        }
        FOTA_DEBUG_PRINTLN("[PATCH] ZLIB patch verified!");
        return true;
    }
    ...
    if (!hpatchi_inplace_open(&ms, patch_mem_read,
        ...
        FOTA_DEBUG_PRINTLN("[PATCH] Invalid patch format (hpatchi_inplace_open)");
        free(patch_buf);
        set_err(err, err_sz, FOTA_TXT_VFY_BAD_PATCH_FORMAT);
        return false;
    }
    if (compress_type != hpi_compressType_no) {
        FOTA_DEBUG_PRINTLN("[PATCH] Compressed patch not supported");
        free(patch_buf);
        set_err(err, err_sz, FOTA_TXT_VFY_COMPRESSED_UNSUPPORTED);
        return false;
    }
    ...
    uint8_t* cache = fota_get_buffer(FOTA_BUF_CAP);
    if (!cache) { free(patch_buf); FOTA_DEBUG_PRINTLN("[PATCH] scratch buffer unavailable"); set_err(err, err_sz, FOTA_TXT_VFY_SCRATCH_BUSY); return false; }
    bool ok = (bool)hpatch_lite_patch(&sl.base, new_size, cache, FOTA_BUF_CAP);
    fota_put_buffer(cache);

    if (!ok) { FOTA_DEBUG_PRINTLN("[PATCH] HPatchLite FAILED"); free(patch_buf); set_err(err, err_sz, FOTA_TXT_VFY_HPATCH_FAILED); return false; }
    ...
    if (memcmp(result_sha, st->new_sha256, 32) != 0) {
        FOTA_DEBUG_PRINT("[PATCH] SHA256 MISMATCH  exp=");
        print_sha16(st->new_sha256); FOTA_DEBUG_PRINTLN("...");
        set_err(err, err_sz, FOTA_TXT_VFY_SHA_MISMATCH);
        return false;
    }
    FOTA_DEBUG_PRINTLN("[PATCH] OK — patch verified!");
    return true;
}
...
bool fota_flash_via_flasher() {
#if !FOTA_HAS_FLASHER
    FOTA_DEBUG_PRINTLN("[FLASHER] flasher_code.h missing.");
    FOTA_DEBUG_PRINTLN("[FLASHER] Run: python nrffota/tools/build_flasher.py");
    return false;
#else
    // ── 0: base FW verification ──
    if (!fota_verify_old_fw()) {
        FOTA_DEBUG_PRINTLN("[FLASHER] ABORTED — base FW mismatch, not risking an overwrite.");
        return false;
    }
    ...
    if (!patch_buf) {
        FOTA_DEBUG_PRINTLN("[FLASHER] patch unavailable (RAM/file)");
        return false;
    }
    if (patch_size == 0 || patch_size > FOTA_FS_FLASH_SIZE) {
        FOTA_DEBUG_PRINTLN("[FLASHER] Invalid patch size: %lu", (unsigned long)patch_size);
        free(patch_buf);
        return false;
    }
    ...
    uint32_t new_fw_size = 0;
    {
        ...
        if (magic == ZPATCH_MAGIC) {
            ...
            FOTA_DEBUG_PRINTLN("[FLASHER] Compressed format: staged=%lu B  raw=%lu B  new_fw=%lu B", (unsigned long)patch_size, (unsigned long)uncomp_sz, (unsigned long)new_fw_size);
            ...
            if (ps) {
                ...
                if (hdr_ok && es > FOTA_MAX_EXTRA_SAFE) {
                    free(patch_buf);
                    FOTA_DEBUG_PRINTLN("[FLASHER] ABORTED — extra_safe %lu > max %lu (flasher would reject, 0xE5)", (unsigned long)es, (unsigned long)FOTA_MAX_EXTRA_SAFE);
                    return false;
                }
            }
        } else {
            ...
            if (!hpatchi_inplace_open(&ms, patch_mem_read,
                                      ...
                                      &uncomp_size, &extra_safe)) {
                free(patch_buf);
                FOTA_DEBUG_PRINTLN("[FLASHER] Invalid patch format");
                return false;
            }
            if (compress_type != hpi_compressType_no) {
                free(patch_buf);
                FOTA_DEBUG_PRINTLN("[FLASHER] Compressed HPatchLite not supported");
                return false;
            }
            new_fw_size = (uint32_t)new_fw_size64;
            FOTA_DEBUG_PRINTLN("[FLASHER] Uncompressed format: new_fw=%lu B  patch=%lu B", (unsigned long)new_fw_size, (unsigned long)patch_size);
            ...
            if (extra_safe > FOTA_MAX_EXTRA_SAFE) {
                free(patch_buf);
                FOTA_DEBUG_PRINTLN("[FLASHER] ABORTED — extra_safe %lu > max %lu (flasher would reject, 0xE5)", (unsigned long)extra_safe, (unsigned long)FOTA_MAX_EXTRA_SAFE);
                return false;
            }
        }
    }
    FOTA_DEBUG_PRINTLN("[FLASHER] Patch in RAM (%lu B)", (unsigned long)patch_size);
    ...
    if (patch_size > 0x20000u) {   //en: 128kB — limit of the patch RAM region (flasher.ld)
        free(patch_buf);
        FOTA_DEBUG_PRINTLN("[FLASHER] Patch > 128kB — does not fit the RAM region");
        return false;
    }
    ...
    if ((uint32_t)(uintptr_t)patch_buf + patch_size > FLASHER_RAM_ADDR) {
        FOTA_DEBUG_PRINTLN("[FLASHER] ABORTED — patch buffer overlaps the flasher RAM region (0x%X+%lu)",
...
bool fota_patch_to_file(char* err, size_t err_sz) {
    FOTA_DEBUG_PRINTLN("[PATCH] HPatchLite not installed (nrffota/hpatchlite/).");
    if (err && err_sz) { strncpy(err, FOTA_TXT_VFY_HPATCHLITE_MISSING, err_sz - 1); err[err_sz - 1] = 0; }
    return false;
}
bool fota_flash_via_flasher() {
    FOTA_DEBUG_PRINTLN("[FLASHER] HPatchLite not installed.");
    return false;
}
...
static void print_step(uint8_t step) {
    ...
    switch (step) {
        ...
        case 0xFE: FOTA_DEBUG_PRINTLN("FAIL — flasher failed and reset"); break;
        default:
            if (step >= 0x20u && step <= 0x3Fu) {
                uint32_t kb = (uint32_t)(step - 0x20u) * 16u;
                FOTA_DEBUG_PRINTLN("puff progress: decompressed ~%lu kB", (unsigned long)kb);
            } else {
...
static void fota_print_flasher_trace() {
    ...
    FOTA_DEBUG_PRINTLN("[FLASHER-TRACE] (unavailable — no trace region)");
#else
    const volatile uint32_t* t = (const volatile uint32_t*)FLASH_TRACE_ADDR;
    if (t[0] == 0xFFFFFFFFu) {
        FOTA_DEBUG_PRINTLN("[FLASHER-TRACE] (empty — flasher wrote no trace)");
        return;
    }
    FOTA_DEBUG_PRINTLN("[FLASHER-TRACE] flasher event sequence:");
    for (uint32_t i = 0; i < FLASH_TRACE_MAX; i++) {
        uint32_t code = t[i];
        if (code == 0xFFFFFFFFu) break;
        //en: "FNV-1a of output" is PARSED by test_nrf-fota/fota_test_lora_repeater.py — keep in sync.
        //sk: "FNV-1a of output" PARSUJE test_nrf-fota/fota_test_lora_repeater.py — drž v synchróne.
        if (code == 0xD0u) { FOTA_DEBUG_PRINTLN("  [chk] FNV-1a of output (new FW) = 0x%X", (unsigned)t[i + 1]); i++; continue; }
...
void fota_print_flasher_debug() {
    ...
    if (s_flasher_step != 0u) {
        FOTA_DEBUG_PRINT("[FLASHER-DBG] last step: ");
...
void fota_debug_decompress() {
    ...
    FotaFile f(FotaFS);
    if (!f.open(FOTA_FS_PATCH, FILE_O_READ)) { FOTA_DEBUG_PRINTLN("[DBG] patch.bin missing"); return; }
    uint32_t sz = (uint32_t)f.size();
    uint32_t magic = 0, uncomp = 0, newfw = 0;
    f.read((uint8_t*)&magic, 4); f.read((uint8_t*)&uncomp, 4); f.read((uint8_t*)&newfw, 4);
    if (magic != 0x42494C5Au) { FOTA_DEBUG_PRINTLN("[DBG] not ZLIB format"); f.close(); return; }
    ...
    FOTA_DEBUG_PRINTLN("[DBG] puff_stream raw=%lu B (exp %lu)  FNV=0x%X  err=%d  %s",
        ...
        (total == uncomp && err == 0) ? "[decomp OK]" : "[DECOMP ERROR!]");
}
